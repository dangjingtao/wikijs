...
wiki.search = async (
  ...
  try {
    ...
    const result: wikiSearchResult = {
      ...
      suggestion: response.query.searchinfo
 ...
 */
wiki.langLinks = async (
  title: string,
  listOptions?: listOptions
): Promise<Array<langLinksResult>> => {
  try {
    if (listOptions?.autoSuggest) {
      title = await setTitleForPage(title);
    }
 ...
 */
wiki.geoSearch = async (
  latitude: number,
  longitude: number,
  geoOptions?: geoOptions
): Promise<Array<geoSearchResult>> => {
  try {
...
wiki.suggest = async (query: string): Promise<string | null> => {
  try {
    ...
    return result.query?.searchinfo?.suggestion
 ...
 */
wiki.random = async (
  format?: randomFormats
): Promise<wikiSummary | title | relatedResult | mobileSections | string> => {
  try {
    if (!format) {
      format = "summary";
    }
